...
#插入破解
if [ -f "split_img/boot.img-dtb" ];then sed -i 's/\x2C\x61\x76\x62/\x00\x00\x00\x00/g' split_img/boot.img-dtb ; fi
...
pwd
if [ -f "ramdisk/fstab.qcom" ];then
  sed -i 's/fileencryption=ice/encryptable=ice/g' ramdisk/fstab.qcom
  sed -i 's/,avb_keys=\/avb\/q-gsi.avbpubkey:\/avb\/r-gsi.avbpubkey:\/avb\/s-gsi.avbpubkey//g' ramdisk/fstab.qcom
  sed -i 's/,avb//g' ramdisk/fstab.qcom
  sed -i 's/=vbmeta_system//g' ramdisk/fstab.qcom
fi
#插入结束
